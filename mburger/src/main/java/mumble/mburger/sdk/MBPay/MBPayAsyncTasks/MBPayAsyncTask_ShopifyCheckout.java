...
public class MBPayAsyncTask_ShopifyCheckout extends AsyncTask<Void, Void, Void> {
     ...
     */
    private String action = MBAPIConstants.ACTION_SHOPIFY_CHECKOUT;

    /**
     * If you wish to use a listener to retrieve the data instead of the ApiListener
     */
    private MBPayApiShopifyCheckoutistener listener;
    ...
    public MBPayAsyncTask_ShopifyCheckout(Context context, MBPayApiShopifyCheckoutistener listener, long order_id, String token) {
        this.weakContext = new WeakReference<>(context);
    ...
    protected void onPostExecute(Void postResult) {
        if (weakContext != null) {
            if (listener == null) {
                ...
                if (error != null) {
                    listener.onShopifyCheckoutError(error);
                } else {
                    listener.onShopifyCheckoutSuccess();
                }
            }
        }
    }
